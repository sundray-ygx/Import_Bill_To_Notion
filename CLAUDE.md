...
A Python-based bill import service that automatically imports payment records from Alipay (支付宝), WeChat Pay (微信支付), and UnionPay (银联) into Notion databases. The application supports three modes: a FastAPI web service for manual uploads, a background scheduler for automated imports, and a CLI for single-file imports.
