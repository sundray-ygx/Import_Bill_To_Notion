...
### 1.1 邮箱账单自动导入 🆕
- 支持从邮箱自动获取账单邮件
- 支持多邮箱配置（QQ、163、Gmail、Outlook、自定义）
- 定时自动检查（每小时/每天/每周）
- 密码加密存储，安全可靠
- 智能去重，避免重复导入
...
| `PASSWORD_ENCRYPTION_KEY` | 邮箱密码加密密钥 | `your-encryption-key` | **是**（邮箱功能） |
| `SCHEDULER_ENABLED` | 是否启用定时任务 | `false` | 否 |
| `EMAIL_CHECK_TIMEOUT` | 邮箱检查超时（秒） | `10` | 否 |
| `EMAIL_MAX_ATTACHMENTS` | 单次最大附件数 | `5` | 否 |
...
2. 点击"邮箱配置"选项卡
3. 点击"添加邮箱"按钮
4. 选择邮箱服务商（QQ、163、Gmail、Outlook或自定义）
5. 填写邮箱配置：
   - 邮箱地址
   - 密码/授权码（注意：QQ、163、Gmail需要使用授权码）
   - IMAP服务器和端口（自动填充）
   - 检查频率（每小时/每天/每周）
6. 点击"测试连接"验证配置
7. 保存配置
8. 系统将自动按设定频率检查邮箱并导入账单
